...
const List = ({ token }) => {
  ...
  const fetchList = async () => {
    try {
      const response = await axios.get('http://localhost:7000/api/product/list', {
  ...
  const removeProducts = async (id) => {
    try {
      const response = await axios.post(
        'http://localhost:7000/api/product/remove',
        { id },
        ...
        {list.length > 0 ? (
          list.map((item) => (
            <div className="flex items-center justify-between py-3 px-4 border-b bg-gray-50 hover:bg-gray-100 transition duration-200" key={item._id}>
              ...
              <p className="flex-1 font-semibold text-gray-900">{item.price}</p>
              <button 
